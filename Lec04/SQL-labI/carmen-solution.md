...
```DESCRIBE country;```
Shows all fields in the table 'country'.
...
```SQL
SELECT Code,Name,Population FROM Country WHERE Continent='Europe' ORDER BY Population ASC LIMIT 1;
```
...
```SQL
SELECT * FROM CountryLanguage WHERE Language='Italian' AND Percentage=100.0;
```
...
```SQL
SELECT Code FROM City WHERE Name LIKE 'Serra%';
SELECT * FROM Country WHERE Code='BRA';
```
**Answer:** ```Serra (in BRA/Brasil)```

**Clue #6:** We're close! Our South American agent says she just got a taxi at the airport, and is headed towards the capital! Look up the country's capital, and get there pronto! Send us the name of where you're headed and we'll follow right behind you!

```SQL
SELECT * FROM City WHERE id=(SELECT Capital FROM Country WHERE Code='BRA');
```
**Answer:** ```Brasília```
